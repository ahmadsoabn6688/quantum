...
def main():
    ...
    # Shorter names for figure labels
    label_map = {
    ...
    plt.scatter(df["runtime_sec"], df["auroc"], s=80)

    for x, y, label in zip(df["runtime_sec"], df["auroc"], labels):
        plt.annotate(label, (x, y), textcoords="offset points", xytext=(5, 5))

    plt.xscale("log")
    plt.xlabel("Runtime in seconds, log scale")
    plt.ylabel("AUROC")
    plt.title("BACE Classification: AUROC vs Runtime")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    out_path = FIG_DIR / "bace_auroc_vs_runtime.png"
